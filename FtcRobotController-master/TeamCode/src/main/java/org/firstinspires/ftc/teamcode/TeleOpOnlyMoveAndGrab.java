...
@TeleOp(name="OnlyMoveAndGrab", group="VT Auto")
public class TeleOpOnlyMoveAndGrab extends LinearOpMode {
...
//
//            LTMotorDC = LTMotorDC * mod * 0.25f;
//            LBMotorDC = LBMotorDC * mod * 0.25f;
//            RTMotorDC = RTMotorDC * mod * 0.25f;
//            RBMotorDC = RBMotorDC * mod * 0.25f;

            double x = gamepad1.left_stick_x * -0.3;
            ...
            double LTMotorDC = (power * cos/max + turn);
            double RTMotorDC = (power * sin/max - turn);
            double LBMotorDC = (power * sin/max + turn);
            double RBMotorDC = (power * cos/max - turn);

            r.OpenPower(LTMotorDC, LBMotorDC, RTMotorDC, RBMotorDC);

            if ((power + Math.abs(turn)) > 1) {
                LTMotorDC = r.LTMotor.getPower() / (power + turn);
                RTMotorDC = r.RTMotor.getPower() / (power + turn);
                LBMotorDC = r.LBMotor.getPower() / (power + turn);
                RBMotorDC = r.RBMotor.getPower() / (power + turn);
                r.OpenPower(LTMotorDC, LBMotorDC, RTMotorDC, RBMotorDC);
            }
